...
- [x] 1.1 Lay down the build system (CMake + Ninja, CMakePresets) and the foundation under
      `d4np/core/` (`status_t`, pluggable `allocator_t` + default allocator).
- [x] 1.2 Implement the first end-to-end slice in `d4np/str/`: `str_view_t` +
      `str_view_split_next` (#15), incl. the spec §3 example.
...
- [x] 2.1 `arena_init` (#1) — initialize a bump-pointer arena over a backing block.
- [x] 2.2 `arena_alloc` (#2) — O(1) allocation with alignment honored.
- [x] 2.3 `arena_reset` (#3) — O(1) bulk free, keeping the main block (+ `arena_destroy`).
- [x] 2.4 `pool_init` (#4) — fixed-block pool over an intrusive free-list.
- [x] 2.5 `pool_alloc` / `pool_free` (#5) — O(1) block alloc/free, fragmentation-free.
...
- [x] 3.1 `vector_t` (#6) — dynamic array, geometric growth, allocator-injected.
- [ ] 3.2 `hashmap_t` (#7) — open addressing + linear probing (cache-friendly).
- [ ] 3.3 `linked_list_t` (#8) — intrusive doubly-linked list.
- [ ] 3.4 `ring_buffer_t` (#9) — thread-safe SPSC circular buffer (verified under TSan).
- [x] 3.5 `string_builder_t` (#10) — efficient dynamic string assembly.
...
- [ ] 4.1 `mutex_t` (#11) — portable shim over pthread/win32.
- [ ] 4.2 `semaphore_t` (#14) — system semaphore (incl. inter-process mode).
- [ ] 4.3 `atomic_queue_t` (#13) — lock-free SPSC over `<stdatomic.h>`.
- [ ] 4.4 `thread_pool_t` (#12) — native threads + internal task queue.
- [ ] 4.5 Stress tests + benchmarks under `bench/concurrency`; whole module green under TSan.

## Milestone 5 — Strings & parsing (`d4np/str`)

- [ ] 5.1 `str_split` (#16) — zero-allocation split into a caller-provided view array.
- [ ] 5.2 `str_parse_int` (#17) — overflow-safe integer parse returning `status_t`.
- [ ] 5.3 `str_parse_float` (#17) — robust float parse with explicit error reporting.

## Milestone 6 — File system & I/O (`d4np/io`)

- [ ] 6.1 `file_read_all` (#18) — read a whole file into an allocator-backed buffer.
- [ ] 6.2 `file_write_all` (#19) — atomic write (temp + rename) with flush.
- [ ] 6.3 `path_combine` (#20) — OS-separator-aware safe path join.

## Milestone 7 — System utilities & diagnostics (`d4np/sys`)

- [ ] 7.1 `error_context_push` / `error_context_pop` (#22) — thread-local error trail.
- [ ] 7.2 `log_write` (#21) — leveled logger (INFO/WARN/ERROR) to console or file.
- [ ] 7.3 `timestamp_ms` (#23) — monotonic millisecond clock.
- [ ] 7.4 `uuid_generate` (#24) — RFC4122 v4 UUID.
- [ ] 7.5 `hash_fnv1a` (#25) — FNV-1a over strings and binary buffers.
...
- [ ] 8.5 Multi-process test harness for `semaphore_t` IPC; perf thresholds gated in CI.
