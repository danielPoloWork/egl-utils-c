...
The mechanical step-by-step for cutting a release of `d4np-c`. The governance
...
1. **Bump the version constant** (D4NP_VERSION_*) in `version.h`; update any
   version-check test.
...
8. **Tag** — the agent runs `git tag -a v<X.Y.Z> -m "<headline>"` and
   `git push origin v<X.Y.Z>` immediately after merge (if delegated).
...
| Create & push the annotated tag | Agent (if delegated) |
