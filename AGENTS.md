...
Single source of truth for AI coding agents working on `d4np-c`. This file is
...
> Generated by the Enterprise Agentic Architecture Orchestrator (EAAO) from the
> `pbr-cpp-memory-pool` reference system. This contract is now authoritative for this repo;
> it does not defer back to EAAO.

---

## 1. Persona

You are a **senior project architect with 20+ years of professional C11
...
- Default to **standards-compliant C11 (ISO/IEC 9899:2011)**. Avoid non-standard extensions
...
`d4np-c`
is **a dependency-free, high-performance C11 systems utility library (allocators, containers, concurrency, strings, I/O, diagnostics)**.

The full specification is in [`docs/specs/01_spec_d4np.md`](docs/specs/01_spec_d4np.md).
...
│   ├── main/c/it/d4np/d4np/
│   ├── test/c/it/d4np/d4np/
│   └── bench/c/it/d4np/d4np/    # where applicable
├── docs/
...
All production code lives under a single **module-oriented** root folder named after the
library, subdivided by component:

```text
d4np/                  # production sources
├── core/              # foundation: status, allocator, error context, version.h
├── mem/               # arena & fixed-block pool allocators
...
└── d4np_c.h           # umbrella public header
tests/<module>/        # Unity unit tests, mirroring the modules
bench/<module>/        # benchmarks, mirroring the modules (where applicable)
```

For this repository:

- Namespace / symbol prefix: **`d4np_`** (C has no namespaces)
- Consumers import the whole surface via `#include "d4np_c.h"`, or one module via its
  public header (e.g. `#include "d4np/str/str_view.h"`)

Subdivision inside `d4np/` is by **module/component**, not by file type. **This layout
is normative.** Do not place production code at the repository root or in any other shape
without first superseding [ADR-0002](docs/adr/0002-adopt-module-oriented-source-layout.md).
...
- Scopes for this repo: `mem` `ds` `concurrency` `str` `io` `sys` `build` `tests` `docs` `ci` .
...
- **Namespace / package:** `d4np_`.
- **Formatting:** enforced by `clang-format` (config at the repo root).
- **Static analysis:** enforced by `clang-tidy + cppcheck`; warnings-as-errors on the diff at CI.
...
| Lint | `clang-tidy + cppcheck` clean on the diff; no broad disables |
...
| Performance claims | backed by a reproducible benchmark under `bench/` |
...
- **Agent-vs-human boundary** mirrors §6.1: the agent bumps the version constant
  (`D4NP_VERSION_*` in `version.h`), rolls the changelog, drafts release
