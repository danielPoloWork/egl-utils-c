...
- **Persona:** senior project architect with 20+ years of enterprise C11
...
- **Source layout:** module-oriented tree. Production code under `d4np/<module>/` (tests
  under `tests/<module>/`, benchmarks under `bench/<module>/`). Symbol prefix `d4np_`. See §5.
...
- **Quality bar:** enterprise — warnings-as-errors, `clang-tidy + cppcheck` clean, `clang-format`
  clean, ASan, UBSan, TSan, Valgrind green, `Doxygen` documented, `consistency_lint.py` passing. No
  shortcuts. See §10.
